...
class Repo implements PaymentRepository {
  ...
  async incrementWalletBalance(ctx: TenantContext, id: string, delta: bigint) { const w = (await this.findCustomerWallet(ctx, id))!; const updated = { ...w, currentBalance: w.currentBalance + delta }; this.wallets.set(id, updated); return updated.currentBalance; }
