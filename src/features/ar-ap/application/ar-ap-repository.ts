...
export interface ArApRepository {
  ...
  markInvoicePosted(ctx:TenantContext,id:string,journalId:string):Promise<Invoice>; markBillPosted(ctx:TenantContext,id:string,journalId:string):Promise<Bill>;
  createPayment(ctx:TenantContext,input:RecordPaymentInput & { direction:"CUSTOMER_PAYMENT"|"VENDOR_PAYMENT"; paymentNumber:string; postedJournalId:string; customerId?:string|null; vendorId?:string|null }):Promise<Payment>;
  updateInvoicePaid(ctx:TenantContext,id:string,paidAmount:bigint,status:Invoice["status"]):Promise<Invoice>; updateBillPaid(ctx:TenantContext,id:string,paidAmount:bigint,status:Bill["status"]):Promise<Bill>;
  createCreditNote(ctx:TenantContext,input:ApplyCreditNoteInput & { noteNumber:string; postedJournalId:string; customerId:string }):Promise<void>;
