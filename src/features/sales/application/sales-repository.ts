...
export interface SalesAuditEvent { action: "SALES_ORDER_CREATED" | "SALES_ORDER_UPDATED" | "SALES_ORDER_CONFIRMED" | "SALES_PAYMENT_ALLOCATED" | "SALES_ORDER_VOIDED"; businessId: string; actorUserId: string; entityType: "sales_order"; entityId?: string; metadata: Record<string, unknown>; }
export interface SalesRepository {
  ...
  updateSalesOrder(ctx: TenantContext, salesOrderId: string, input: CreateSalesOrderInput, computed: { subtotal: bigint; discountTotal: bigint; taxTotal: bigint; totalAmount: bigint; items: Array<{ businessId: string; productId: string; productType: "PHYSICAL" | "DIGITAL" | "SERVICE"; quantity: bigint; unitPrice: bigint; discountAmount: bigint; taxAmount: bigint; lineTotal: bigint; locationId?: string | null; providerProductId?: string | null }> }): Promise<SalesOrderEntity>;
  deleteSalesOrder(ctx: TenantContext, salesOrderId: string): Promise<boolean>;
  deleteAnySalesOrder(ctx: TenantContext, salesOrderId: string): Promise<boolean>;
...
export interface UpdateSalesOrderCommand extends CreateSalesOrderInput, Meta { salesOrderId: string; }
...
export interface DeleteSalesOrderCommand { businessId: string; salesOrderId: string; actorUserId: string; requestId?: string; ipAddress?: string; userAgent?: string; }
export interface VoidSalesOrderCommand extends DeleteSalesOrderCommand { reason: string; }
