...
export default function Page() {
  ...
  return <div className="grid gap-6"><WorkspaceHeader eyebrow="Purchase" title="Purchase Returns" description="Return received goods to suppliers and post the reversing inventory/GRNI journal through PurchaseService." /><SplitWorkspace main={<GlassTable columns={[{ key: "ret", header: "Return" }, { key: "po", header: "PO" }, { key: "date", header: "Date" }, { key: "cost", header: "Total cost" }, { key: "journal", header: "Journal" }]} rows={rows} empty="No returns loaded" />} side={<><GlassPanel><div className="grid gap-4"><label className="grid gap-1"><span className="text-xs text-muted">Purchase order</span><select value={orderId} onChange={(e) => pickOrder(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8"><option value="">{returnable.length ? "Pilih PO" : "Tidak ada PO yang sudah diterima"}</option>{returnable.map((o: any) => <option key={o.id} value={o.id}>{o.orderNumber ?? o.id} · {o.status}</option>)}</select></label><label className="grid gap-1"><span className="text-xs text-muted">Return date</span><input value={returnDate} onChange={(e) => setReturnDate(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8" /></label><label className="grid gap-1"><span className="text-xs text-muted">Reason</span><input value={reason} onChange={(e) => setReason(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8" placeholder="Barang rusak / salah kirim" /></label><label className="grid gap-1"><span className="text-xs text-muted">Location (gudang)</span><input list="return-locations" value={location} onChange={(e) => setLocation(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8" placeholder="main" /><datalist id="return-locations">{knownLocations.map((l: any) => <option key={l} value={l} />)}</datalist></label>{orderItems.length > 0 ? <div className="grid gap-2">{orderItems.map((it: any) => <div key={it.productId} className="grid items-end gap-2 md:grid-cols-[2fr_1fr]"><label className="grid gap-1"><span className="text-xs text-muted">{productName(it.productId)}</span><span className="text-xs text-muted">Received {String(it.receivedQuantity ?? 0)}</span></label><input value={qty[it.productId] ?? ""} onChange={(e) => setQty((p) => ({ ...p, [it.productId]: e.target.value }))} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm tabular-nums dark:bg-white/8" placeholder="0" /></div>)}</div> : null}<button type="button" onClick={() => void submit()} className="h-10 rounded-md bg-foreground px-4 text-sm font-medium text-background">Create return</button>{error ? <p role="alert" className="text-sm text-danger">{error}</p> : null}{okMsg ? <p role="status" className="text-sm text-success">{okMsg}</p> : null}</div></GlassPanel><DetailPanel title="Accounting impact">Returns reverse the GRNI/inventory entry and reduce on-hand stock for the returned quantities.</DetailPanel></>} /></div>;
}
