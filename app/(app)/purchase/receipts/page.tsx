...
export default function Page() {
  ...
  return <div className="grid gap-6"><WorkspaceHeader eyebrow="Purchase" title="Purchase Receipts" description="Receive purchase orders and post inventory/GRNI impact through PurchaseService and InventoryService." /><SplitWorkspace main={<GlassTable columns={[{ key: "receipt", header: "Receipt" }, { key: "po", header: "PO" }, { key: "date", header: "Date" }, { key: "cost", header: "Total cost" }, { key: "journal", header: "Journal" }]} rows={rows} empty="No receipts loaded" />} side={<><GlassPanel><div className="grid gap-4"><label className="grid gap-1"><span className="text-xs text-muted">Purchase order</span><select value={orderId} onChange={(e) => pickOrder(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8"><option value="">{receivable.length ? "Pilih PO" : "Tidak ada PO yang bisa diterima"}</option>{receivable.map((o: any) => <option key={o.id} value={o.id}>{o.orderNumber ?? o.id} · {o.status}</option>)}</select></label><label className="grid gap-1"><span className="text-xs text-muted">Receipt date</span><input value={receiptDate} onChange={(e) => setReceiptDate(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8" /></label><label className="grid gap-1"><span className="text-xs text-muted">Location (gudang)</span><input list="receipt-locations" value={location} onChange={(e) => setLocation(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8" placeholder="main" /><datalist id="receipt-locations">{knownLocations.map((l: any) => <option key={l} value={l} />)}</datalist></label>{orderItems.length > 0 ? <div className="grid gap-2">{orderItems.map((it: any) => <div key={it.productId} className="grid items-end gap-2 md:grid-cols-[2fr_1fr]"><label className="grid gap-1"><span className="text-xs text-muted">{productName(it.productId)}</span><span className="text-xs text-muted">Ordered {String(it.quantity)} · received {String(it.receivedQuantity ?? 0)}</span></label><input value={qty[it.productId] ?? ""} onChange={(e) => setQty((p) => ({ ...p, [it.productId]: e.target.value }))} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm tabular-nums dark:bg-white/8" placeholder="0" /></div>)}</div> : null}<button type="button" onClick={() => void submit()} className="h-10 rounded-md bg-foreground px-4 text-sm font-medium text-background">Receive order</button>{error ? <p role="alert" className="text-sm text-danger">{error}</p> : null}{okMsg ? <p role="status" className="text-sm text-success">{okMsg}</p> : null}</div></GlassPanel><DetailPanel title="Inventory impact">Receipt lines update stock balances and average cost through existing inventory flows.</DetailPanel></>} /></div>;
}
