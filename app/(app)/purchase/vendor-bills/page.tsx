...
export default function Page() {
  ...
  return <div className="grid gap-6"><WorkspaceHeader eyebrow="Purchase" title="Vendor Bills" description="Generate vendor bills from received purchase orders and post AP through ArApService integration." /><SplitWorkspace main={<GlassTable columns={[{ key: "bill", header: "Bill" }, { key: "vendor", header: "Vendor" }, { key: "po", header: "PO" }, { key: "due", header: "Due" }, { key: "amount", header: "Amount" }]} rows={rows} empty="No vendor bills loaded" />} side={<><GlassPanel><div className="grid gap-4"><label className="grid gap-1"><span className="text-xs text-muted">Purchase order (received)</span><select value={orderId} onChange={(e) => setOrderId(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8"><option value="">{billable.length ? "Pilih PO" : "Tidak ada PO berstatus RECEIVED"}</option>{billable.map((o: any) => <option key={o.id} value={o.id}>{o.orderNumber ?? o.id} · {vendorName(o.supplierId)}</option>)}</select></label><label className="grid gap-1"><span className="text-xs text-muted">Bill date</span><input value={billDate} onChange={(e) => setBillDate(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8" /></label><label className="grid gap-1"><span className="text-xs text-muted">Due date</span><input value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8" /></label><label className="grid gap-1"><span className="text-xs text-muted">Description</span><input value={description} onChange={(e) => setDescription(e.target.value)} className="h-10 rounded-md border border-border bg-white/60 px-2 text-sm dark:bg-white/8" placeholder="Tagihan pembelian" /></label><button type="button" onClick={() => void submit()} className="h-10 rounded-md bg-foreground px-4 text-sm font-medium text-background">Generate bill</button>{error ? <p role="alert" className="text-sm text-danger">{error}</p> : null}{okMsg ? <p role="status" className="text-sm text-success">{okMsg}</p> : null}</div></GlassPanel><DetailPanel title="AP integration">Generating a bill posts DR GRNI / CR AP, creates the AP bill via ArApService, and marks the order COMPLETED.</DetailPanel></>} /></div>;
}
