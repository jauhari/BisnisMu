...
export default function Page() {
  ...
  return <ReportWorkspace title="Laporan Penjualan" startsOn={startsOn} endsOn={endsOn} onStartsOnChange={setStartsOn} onEndsOnChange={setEndsOn} activePreset={activePreset} onPresetChange={setActivePreset}><section className="grid gap-4 md:grid-cols-4"><GlassStatsCard title="Pesanan" value={formatNumber(report.orderCount ?? 0)} detail="Dalam periode" /><GlassStatsCard title="Item Terjual" value={formatNumber(report.itemCount ?? 0)} detail="Terjual" /><GlassStatsCard title="Total" value={formatRupiah(report.totalAmount ?? 0)} detail="Bruto" /><GlassStatsCard title="Belum Bayar" value={formatRupiah(report.outstandingAmount ?? 0)} detail="Belum lunas" /></section><GlassTable tableId="report-sales" columns={[{ key: "status", header: "Status" }, { key: "count", header: "Jumlah" }, { key: "total", header: "Total" }]} rows={Object.entries(report.byStatus ?? {}).map(([status, row]: any) => ({ status, count: formatNumber(row.count ?? 0), total: formatRupiah(row.totalAmount ?? 0) }))} empty="Tidak ada data penjualan." /></ReportWorkspace>;
}
