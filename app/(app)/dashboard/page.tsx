...
export default function Page() {
  ...
  return <DashboardGrid>
    ...
    <KpiGrid items={[{ title: "Sales today", value: formatMoney(overview.sales?.salesToday ?? 0), detail: "Live" }, { title: "Net profit", value: formatMoney(overview.profitability?.netProfit ?? 0), detail: `Margin ${overview.profitability?.profitMargin ?? 0}%` }, { title: "Cash + bank", value: formatMoney(Number(overview.cash?.cashOnHand ?? 0) + Number(overview.cash?.bankBalance ?? 0)), detail: "Available balance" }, { title: "Inventory value", value: formatMoney(overview.inventory?.inventoryValue ?? 0), detail: `${overview.inventory?.lowStockItems?.length ?? 0} low stock alerts` }]} />
