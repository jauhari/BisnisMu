...
type AccountNode = { id: string; code: string; name: string; groupCode: number | string; normalBalance: "DEBIT" | "CREDIT"; parentCode?: string | null; isActive: boolean; isPostingAllowed?: boolean; children?: AccountNode[] };
...
function CoaFormFields({ form, rows }: { form: ManagedFormRenderProps<CoaForm>["form"]; rows: TreeRow[] }) {
  ...
  }, [form, groupCode, parentCode, rows]);
  return <div className="grid gap-4"><label className="grid gap-2"><span className="text-xs font-medium uppercase tracking-wide text-muted">Kode akun</span><GlassInput value={form.watch("code")} readOnly className="h-11 bg-slate-950/5 text-muted" /></label><RhfTextField<CoaForm> name="name" label="Nama akun" placeholder="Bank BRI / Kas Operasional" /><RhfDataSelect<CoaForm> name="groupCode" label="Grup" options={GROUP_OPTIONS} placeholder="Pilih grup" /><RhfDataSelect<CoaForm> name="parentCode" label="Induk akun" options={[{ value: "", label: "Tanpa induk" }, ...parentOptions]} placeholder="Pilih induk akun" /><RhfDataSelect<CoaForm> name="isPostingAllowed" label="Tipe akun" options={[{ value: "false", label: "Header / kategori" }, { value: "true", label: "Posting / dipakai jurnal" }]} placeholder="Pilih tipe akun" /><div className="grid gap-2"><span className="text-xs font-medium uppercase tracking-wide text-muted">Saldo normal</span><div className="rounded-md border border-border bg-slate-950/5 px-3 py-2.5 text-sm font-medium text-muted dark:bg-white/8">{form.watch("normalBalance")}</div><p className="text-xs leading-5 text-muted">Otomatis dari grup akun. Aset/Beban normalnya DEBIT; Liabilitas/Ekuitas/Pendapatan normalnya CREDIT.</p></div><p className="text-xs leading-5 text-muted">{isPostingAllowed === "false" ? "Header dipakai untuk membuat kategori seperti Kas atau Bank." : "Akun posting dipakai untuk transaksi jurnal."} Kode otomatis mengikuti induk dan nomor terakhir.</p><button type="submit" className="h-10 rounded-md bg-foreground px-4 text-sm font-medium text-background">Simpan akun</button></div>;
}
export default function Page() {
  ...
  const { data, isLoading, error } = useListQuery<AccountNode[]>("/api/accounting/chart-of-accounts", ["list", "accounting-coa"]);
