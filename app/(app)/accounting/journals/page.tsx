...
export default function Page() {
  ...
  const resetForm = () => { setEditingDraft(null); setDescription("Manual adjustment"); setLines([{ accountId: "", side: "DEBIT", amount: "" }, { accountId: "", side: "CREDIT", amount: "" }]); setFormOpen(false); };
  ...
  async function saveDraft() {
    ...
    try {
      ...
      setOkMsg("Draft tersimpan. Masih bisa diedit sebelum post."); setFormOpen(false); await journals.refetch?.();
  ...
  return <div className="grid gap-6"><WorkspaceHeader eyebrow="Akuntansi" title="Jurnal" description="Jurnal manual untuk koreksi, penyesuaian akhir periode, reversal, dan transaksi khusus." action={<button type="button" onClick={() => { resetForm(); setFormOpen(true); }} className="h-10 rounded-lg bg-foreground px-4 text-sm font-medium text-background">Tambah jurnal manual</button>} />{okMsg ? <p role="status" className="rounded-md border border-success/25 bg-success/8 px-3 py-2 text-sm text-success">{okMsg}</p> : null}<GlassTable selectable={false} tableId="accounting-journals" columns={[{ key: "tanggal", header: "Tanggal" }, { key: "nomor", header: "No. Jurnal" }, { key: "status", header: "Status", render: (row: any) => <span className={row.status === "DRAFT" ? "rounded bg-warning/15 px-2 py-1 text-xs font-medium text-warning" : row.status === "REVERSED" ? "rounded bg-danger/10 px-2 py-1 text-xs font-medium text-danger" : "rounded bg-success/12 px-2 py-1 text-xs font-medium text-success"}>{row.status}</span> }, { key: "account", header: "Akun" }, { key: "debit", header: "Debit", render: (row: any) => row.debit }, { key: "credit", header: "Kredit", render: (row: any) => row.credit }, { key: "memo", header: "Memo" }, { key: "actions", header: "Aksi", render: (row: any) => <div className="flex flex-wrap gap-1.5">{row.status === "DRAFT" ? <><button type="button" onClick={() => void rowAction("edit", row)} className="rounded border border-border px-2 py-1 text-xs">Edit</button><button type="button" onClick={() => void rowAction("post", row)} className="rounded bg-foreground px-2 py-1 text-xs text-background">Post</button><button type="button" onClick={() => void rowAction("delete", row)} className="rounded border border-danger/40 px-2 py-1 text-xs text-danger">Delete</button></> : <><button type="button" onClick={() => void rowAction("copy", row)} className="rounded border border-border px-2 py-1 text-xs">Copy</button>{row.status === "POSTED" ? <button type="button" onClick={() => void rowAction("reverse", row)} className="rounded border border-danger/40 px-2 py-1 text-xs text-danger">Reverse</button> : null}</>}</div> }]} rows={rows} empty="Belum ada jurnal" /><FormDrawer open={formOpen} onClose={() => { setFormOpen(false); if (!editingDraft) resetForm(); }} title={editingDraft ? "Edit draft jurnal" : "Tambah jurnal manual"} subtitle="Dipakai untuk koreksi, penyesuaian, reversal, saldo awal, atau transaksi khusus."><div className="grid gap-4"><div className="grid gap-4 sm:grid-cols-[minmax(180px,220px)_1fr]"><label className="grid gap-2"><span className="text-xs font-medium uppercase tracking-wide text-muted">Tanggal transaksi</span><GlassDatePicker value={transactionDate} onChange={setTransactionDate} className="h-11 w-full" /></label><label className="grid gap-2"><span className="text-xs font-medium uppercase tracking-wide text-muted">Keterangan</span><GlassInput value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Koreksi / penyesuaian / saldo awal" className="h-11" /></label></div>{editingDraft ? <div className="rounded-md border border-accent/30 bg-accent/8 px-3 py-2 text-sm text-accent">Mengedit draft {editingDraft.journalNumber}. Jurnal posted tidak diedit langsung.</div> : null}<div className="grid gap-3">{lines.map((line, i) => <div key={i} className="grid items-end gap-2 grid-cols-1 sm:grid-cols-[minmax(0,1fr)_120px] lg:grid-cols-[minmax(0,1fr)_120px_minmax(140px,160px)_72px]"><label className="grid gap-1"><span className="text-xs text-muted">Akun</span><GlassDataSelect value={line.accountId} onChange={(v) => update(i, { accountId: v })} placeholder="Pilih akun" options={postable.map((a) => ({ value: a.id, label: `${a.code} ${a.name}`, code: a.code, name: a.name, groupLabel: ACCOUNT_GROUP_LABELS[a.groupCode] ?? `Grup ${a.groupCode}`, normalBalance: a.normalBalance }))} className="h-10" /></label><label className="grid gap-1"><span className="text-xs text-muted">Posisi</span><GlassDataSelect value={line.side} onChange={(v) => update(i, { side: v as Line["side"] })} options={[{ value: "DEBIT", label: "Debit" }, { value: "CREDIT", label: "Kredit" }]} className="h-10" /></label><label className="grid gap-1 sm:col-span-2 lg:col-span-1"><span className="text-xs text-muted">Nominal</span><GlassInput value={formatAmount(line.amount)} onChange={(e) => update(i, { amount: digitsOnly(e.target.value) })} inputMode="numeric" className={`h-10 font-medium ${amountTone(line.side)}`} placeholder="0" /></label><button type="button" onClick={() => removeLine(i)} disabled={lines.length <= 2} className="h-10 rounded-md border border-border px-3 text-sm text-muted transition hover:border-muted hover:text-foreground disabled:opacity-40 disabled:hover:border-border disabled:hover:text-muted sm:justify-self-start lg:justify-self-stretch">Hapus</button></div>)}</div><div className="flex flex-wrap gap-2"><button type="button" onClick={addLine} className="h-10 rounded-md border border-border px-4 text-sm">+ Tambah baris</button><button type="button" disabled={busy} onClick={() => void saveDraft()} className="h-10 rounded-md border border-border px-4 text-sm font-medium disabled:opacity-50">Simpan draft</button><button type="button" disabled={busy} onClick={() => void submitPosted()} className="h-10 rounded-md bg-foreground px-4 text-sm font-medium text-background disabled:opacity-50">Posting jurnal</button>{editingDraft ? <button type="button" disabled={busy} onClick={resetForm} className="h-10 rounded-md px-4 text-sm text-muted disabled:opacity-50">Batal edit</button> : null}</div>{error ? <p role="alert" className="text-sm text-danger">{error}</p> : null}{okMsg ? <p role="status" className="text-sm text-success">{okMsg}</p> : null}<JournalBalanceBar debit={BigInt(debit)} credit={BigInt(credit)} /></div></FormDrawer></div>;
}
