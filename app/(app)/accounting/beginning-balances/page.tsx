...
export default function Page() {
  ...
  return <div className="grid gap-6"><WorkspaceHeader eyebrow="Accounting" title="Beginning Balances" description="Save and post balanced beginning balances through BusinessService and JournalPostingService." /><GlassPanel><div className="grid gap-4"><label className="grid gap-2 md:max-w-sm"><span className="text-xs font-medium uppercase tracking-wide text-muted">Fiscal period</span><GlassDataSelect value={fiscalPeriodId} onChange={setFiscalPeriodId} placeholder={periodList.length ? "Pilih fiscal period" : "Belum ada fiscal period"} options={periodList.map((p: any) => ({ value: p.id, label: p.fiscalYear }))} /></label><div className="grid gap-3">{lines.map((line, i) => <div key={i} className="grid items-end gap-2 md:grid-cols-[2fr_1fr_1fr_auto]"><label className="grid gap-1"><span className="text-xs text-muted">Account</span><GlassDataSelect value={line.accountId} onChange={(v) => update(i, { accountId: v })} placeholder="Pilih akun" options={postable.map((a) => ({ value: a.id, label: `${a.code} ${a.name}` }))} className="h-10" /></label><label className="grid gap-1"><span className="text-xs text-muted">Side</span><GlassDataSelect value={line.side} onChange={(v) => update(i, { side: v as Line["side"] })} options={[{ value: "DEBIT", label: "DEBIT" }, { value: "CREDIT", label: "CREDIT" }]} className="h-10" /></label><label className="grid gap-1"><span className="text-xs text-muted">Amount</span><GlassInput value={line.amount} onChange={(e) => update(i, { amount: e.target.value })} className="h-10" placeholder="0" /></label><button type="button" onClick={() => removeLine(i)} disabled={lines.length <= 1} className="h-10 rounded-md border border-border px-3 text-sm disabled:opacity-40">Hapus</button></div>)}</div><div className="flex gap-2"><button type="button" onClick={addLine} className="h-10 rounded-md border border-border px-4 text-sm">+ Tambah baris</button><button type="button" onClick={() => void submit()} className="h-10 rounded-md bg-foreground px-4 text-sm font-medium text-background">Save balances</button></div>{error ? <p role="alert" className="text-sm text-danger">{error}</p> : null}{okMsg ? <p role="status" className="text-sm text-success">{okMsg}</p> : null}<JournalBalanceBar debit={BigInt(debit)} credit={BigInt(credit)} /></div></GlassPanel><GlassTable columns={[{ key: "account", header: "Account" }, { key: "side", header: "Side" }, { key: "amount", header: "Amount" }, { key: "status", header: "Status" }]} rows={rows} empty="No beginning balance lines" /></div>;
}
